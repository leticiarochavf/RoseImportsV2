...
const allowedProjectRef = process.env.BULK_IMPORT_TEST_PROJECT_REF;
const isLocal = ["127.0.0.1", "localhost"].includes(parsedUrl.hostname);
const isAuthorizedDisposableProject =
  allowedProjectRef && parsedUrl.hostname === `${allowedProjectRef}.supabase.co`;
if (!isLocal && !isAuthorizedDisposableProject) {
  throw new Error(
    `Recusado: host ${parsedUrl.hostname} não é local nem o projeto descartável autorizado.`,
  );
}
...
for (const analysis of analyses) {
  ...
  if (analysis.proposedAction !== "create_inactive_product" || !analysis.productType) {
    continue;
  }

  const categoryId = analysis.categorySlug
    ? categoryBySlug.get(analysis.categorySlug)
    : undefined;
  ...
  while (usedSlugs.has(slug)) {
    ...
    slug = `${analysis.slug.slice(0, 80 - suffixText.length)}${suffixText}`;
    suffix += 1;
  }
  usedSlugs.add(slug);
