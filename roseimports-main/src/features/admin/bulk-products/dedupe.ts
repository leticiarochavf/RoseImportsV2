...
function analyzeRecord(
  ...
  if (hasInsufficientProductIdentity(record)) {
    return result(record, {
      status: "incomplete",
      reasons: ["insufficient_product_identity"],
    });
  }

  if (!record.brand) {
    const nameAndVolumeCandidates = catalog.filter(
      (candidate) =>
    ...
    if (nameAndVolumeCandidates.length > 0) {
      return result(record, {
        status: "possible_duplicate",
        reasons: ["brand_missing_requires_review"],
        candidates: nameAndVolumeCandidates,
      });
    }
