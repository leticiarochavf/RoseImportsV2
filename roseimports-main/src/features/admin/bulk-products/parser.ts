...
export function parseBulkProducts(input: string): ParsedBulkProduct[] {
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex += 1) {
    ...
    if (isUppercaseHeading(source)) continue;
...
function buildRecord({
  ...
  const name = normalizeDisplayCase(rawName);
  ...
  return {
    ...
    concentration,
    productType,
    categorySlug: readCategorySlug(section, productType),
...
function readCategorySlug(
  section: Section | null,
  productType: BulkProductType | null,
): BulkProductCategorySlug | null {
  if (section === "body_splash" || section === "cosmetico") return "cosmeticos";
  if (section === "kit" || section === "perfume") return "perfumes";
  if (productType === "body_splash" || productType === "cosmetico") return "cosmeticos";
...
function buildVariantLabel({
  ...
  if (isKit) return "Kit";
  if (isDecant && volumeMl !== null) return `Decant ${volumeMl} ml`;

  const structured = [
    concentration,
    volumeMl === null ? null : `${volumeMl} ml`,
  ].filter((part): part is string => part !== null);

  return structured.length > 0 ? structured.join(" ") : name;
}
...
function isUppercaseHeading(value: string): boolean {
  if (/^\d/.test(value) || !/[\p{L}]/u.test(value)) return false;
...
function normalizeDisplayCase(value: string): string {
  if (!isMostlyUppercase(value)) return collapseWhitespace(value);
