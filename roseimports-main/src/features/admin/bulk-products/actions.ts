...
export async function analyzeBulkProducts(
  ...
  if (catalogResult.error || categoriesResult.error) {
    return {
      ok: false,
  ...
  const catalog = mapCatalogCandidates(
    (catalogResult.data ?? []) as unknown as CatalogProductRow[],
  );
  ...
  const usedSlugs = new Set(
    ((catalogResult.data ?? []) as unknown as CatalogProductRow[]).map(
  ...
  return rows.map((product) => {
    ...
    return {
      ...
      variants: product.product_variants.map((variant) => ({
        ...
        isKit: variant.is_kit || /^kits?\b/i.test(product.name),
...
function translateBulkImportError(message: string): string {
